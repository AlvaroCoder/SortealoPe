...
export default function Root() {
  const { idRaffle } = useLocalSearchParams();
  ...
  useEffect(() => {
    function getDataRaffle() {
      ...
      const dataFilter = data.find((raffle) => raffle?.id === parseInt(idRaffle));      
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.imageContainer}>
        <Image
          style={styles.image}
          source={{ uri: dataRaffle?.urlImagen }}
          resizeMode='cover'
        />
      </View>
      ...
      <View style={styles.headerContainer}>
        ...
        <View style={styles.priceContainer}>
          <Title2 style={styles.priceText}>S/. {dataRaffle?.price}</Title2>
        </View>
      </View>
      ...
      <View style={styles.infoContainer}>
        <View style={styles.infoRow}>
          ...
          />
          <Text style={styles.infoText}>{dataRaffle?.date}</Text>
        </View>
        <View style={styles.infoRow}>
          <Ionicons
            ...
            color={Colors.principal.red[500]}
          />
          <Text style={styles.infoText}>{dataRaffle?.location}</Text>
        </View>
      </View>
      ...
      <View style={styles.statsContainer}>
        <View style={styles.statItem}>
          <View style={styles.statIconContainer}>
            <Ionicons name="people-outline" size={20} color={Colors.principal.red[500]} />
          </View>
          <View style={styles.statContent}>
            <Text style={styles.statNumber}>{dataRaffle?.sellers || 0}</Text>
        ...
        <View style={styles.statItem}>
          <View style={styles.statIconContainer}>
            <Ionicons name="person-outline" size={20} color={Colors.principal.red[500]} />
          </View>
          <View style={styles.statContent}>
            <Text style={styles.statNumber}>{dataRaffle?.clients || 0}</Text>
        ...
        <View style={styles.statItem}>
          <View style={styles.statIconContainer}>
            <Ionicons 
              name={dataRaffle?.status === 'Iniciado' ? 'play-circle' : 'time'} 
              size={20} 
              color={Colors.principal.red[500]} 
            />
          </View>
          <View style={styles.statContent}>
            <Text style={[
              styles.statStatus,
              { color: dataRaffle?.status === 'Iniciado' ? Colors.success : Colors.warning }
            ]}>
              {dataRaffle?.status || 'Pendiente'}
            </Text>
...
const styles = StyleSheet.create({
  ...
  descriptionContainer: {
    ...
    borderRadius: 12,
    borderLeftWidth: 4,
    borderLeftColor: Colors.principal.red[300],
  },
  descriptionTitle: {
    fontSize: 16,
    fontWeight: '700',
